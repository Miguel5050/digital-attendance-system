...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'student') {
    ...
    $c = $_COOKIE['PHPSESSID'] ?? 'null_cookie';
    echo json_encode(['success' => false, 'error' => "Unauthorized_Debug: Role=$r, Cookie=$c"]);
    exit();
}
...
if (!$code || !$lat || !$lng) {
    echo json_encode(['success' => false, 'error' => 'Missing required data (Location or Code).']);
    exit();
}

// 1. Get Active Session Context purely by code
$stmt = $pdo->prepare("
    SELECT s.id, s.expires_at, cg.room_lat, cg.room_lng, cg.course_id, cg.group_id
...
if (!$session) {
    echo json_encode(['success' => false, 'error' => 'Invalid or expired attendance code.']);
    exit();
}
...
$distance = haversineGreatCircleDistance($lat, $lng, $session['room_lat'], $session['room_lng']);
// If room has coordinates constraint, ensure < 50 meters
if ($session['room_lat'] && $session['room_lng']) {
    if ($distance > 50) {
        ...
        echo json_encode(['success' => false, 'error' => 'GPS Verification failed. You are ' . round($distance) . 'm away from the classroom.']);
        exit();
    }
}
